...
<div align="center"><img src="docs/images/architecture.svg" width="92%"></div>
...
erDiagram
    ...
    MEMBER  ||--o{ LOGIN   : logs
    MEMBER  ||--o{ PRODUCT : owns
    MEMBER  ||--o{ PAYMENT : pays
    BOARD   ||--o{ COMMENT : has
    BOARD   ||--o{ CODE    : has
    PRODUCT ||--o{ PAYMENT : in
    TEAM    ||--o{ PROJECT : has
    PROJECT ||--o{ RESUME  : gets
```
...
sequenceDiagram
    ...
    B->>S: 로그인 (ID/PW · BCrypt 검증)
    B->>I: 접속 IP 조회
    I-->>B: IP · 도시 · ISP
    B->>S: POST /log (로그인 이력 저장)
    S-->>B: 세션 발급 · 마이페이지
```
