...
**시작하기** &nbsp; 1. [실행](#실행) · 2. [기술 스택](#기술-스택) · 3. [둘러보기](#둘러보기) · 4. [팀](#팀)

**설계** &nbsp; 5. [시스템 구조](#시스템-구조) · 6. [프로젝트 구조](#프로젝트-구조)

**기여 & 회고** &nbsp; 7. [내가 맡은 부분](#내가-맡은-부분) · 8. [기억에 남는 문제들](#기억에-남는-문제들) · 9. [개선 내역](#이후-개선한-것) · 10. [한계](#한계)
...
| 프로필 | 멤버 | 담당 |
| :---: | :---: | :--- |
| <a href="https://github.com/SanghyeokLee-KR"><img src="https://github.com/SanghyeokLee-KR.png" width="60"></a> | **[이상혁](https://github.com/SanghyeokLee-KR)**<br>*(본인)* | **PM · ERD 설계** · 인증/보안(Spring Security) · 결제(카카오페이) · 실시간 채팅(WebSocket) · AI 챗봇(Dialogflow) |
| <a href="https://github.com/gomdi"><img src="https://github.com/gomdi.png" width="60"></a> | **[김보겸](https://github.com/gomdi)** | Bootstrap 템플릿 기반 페이지 구성·정리, 네이밍 규칙, 랜딩·소개·문의 페이지 |
| — | **유기민** | UI 컴포넌트 공유 페이지 (HTML/CSS 컴포넌트·미리보기) |
| — | **하진철** | 게시판(목록·작성·상세·댓글), 구인구직(팀 모집·이력서) |

## 시스템 구조

`Controller → Service → Repository → Entity` 의 계층 구조입니다. 트랜잭션 경계는 Service에 두고, 외부 API(챗봇·결제·위치 조회)도 Service에서 호출합니다.

<div align="center"><img src="docs/images/architecture-overview.svg" width="100%"></div>

데이터 모델 — 19개 테이블의 핵심 관계 (ERD):

<div align="center"><img src="docs/images/data-model.svg" width="92%"></div>
...
팀에서 역할을 나눠 작업했고, 아래는 제가 직접 구현한 부분입니다.

### 회원가입 · 로그인 (프론트 + 백엔드)

이메일 인증 회원가입과 세션 로그인을 화면부터 서버 로직까지 구현했습니다. 회원가입은 **기본정보 → 이메일 인증 → 추가정보** 3단계 폼이 **페이드 인/아웃 애니메이션**으로 부드럽게 전환됩니다. 비밀번호는 BCrypt로 해싱하고, 로그인에 성공하면 세션에 사용자 정보를 담습니다.

<img src="docs/images/signup-1.png" width="32%"> <img src="docs/images/signup-3-info.png" width="32%"> <img src="docs/images/login.png" width="32%">

```css
/* signup.css — 단계 전환 페이드 인/아웃 */
...
```
`MemberController` `MemberService` `member/signup.html` · `signup.css`
...
**프로필 사진이 한 박자 늦게 떴습니다.** 업로드 파일을 `src` 폴더에 저장했는데, 실행 중인 서버는 `build` 폴더(classpath)에서 정적 파일을 읽고 있었습니다. 두 위치가 달라서 방금 올린 사진이 바로 안 보였습니다. 저장과 서빙 위치를 외부 `uploads` 폴더로 통일하고 `ResourceHandler`로 매핑하니 바로 뜨고, JAR로 묶어도 동작합니다.

**음수를 보내면 포인트가 오히려 늘었습니다.** 차감은 `현재 - 차감액` 인데 차감액이 음수면 더해지는 셈이었고, `현재 >= 음수` 조건도 항상 통과했습니다. 차감 값이 0 이하면 막도록 검증을 넣었습니다.

**Security를 켜니 앱이 전부 막혔습니다.** 세션 커스텀 로그인을 쓰는데 Spring Security엔 인증 주체가 없어 로그인한 사용자까지 차단됐습니다. 세션을 `SecurityContext`로 올리는 필터를 끼워 둘을 연결했습니다.
